...
- **`/src/templates/`**: Shared HTML layout template (`layout.html`)
- **`/src/homepage/`**: Homepage content fragments (`en.html`, `fr.html`)
- **`/src/blog/`**: Blog markdown posts in English (`*.md`)
- **`/src/blog/fr/`**: Blog markdown posts in French (`*.md`)
- **`build.py`**: Build script — generates all HTML from `src/` into root
- **Root**: English homepage (`index.html` — generated)
- **`/fr/`**: French homepage (`fr/index.html` — generated)
- **`/blog/`**: Generated blog HTML (indexes + post pages)
...
1. **Always update the English version first** (homepage: `src/homepage/en.html`, blog: `src/blog/*.md`)
2. **Then update the French version** (homepage: `src/homepage/fr.html`, blog: `src/blog/fr/*.md`)
...
Blog posts are written and edited in English in `src/blog/*.md`. French translations are maintained in `src/blog/fr/*.md` with the same filenames.

**IMPORTANT**: When a blog post is created or edited:
1. Write/edit the English version in `src/blog/*.md`
2. Create/update the matching French translation in `src/blog/fr/*.md` (same filename)
...
- Edit content in `src/homepage/*.html` (homepage) or `src/blog/*.md` (blog posts)
- Edit shared layout in `src/templates/layout.html`
